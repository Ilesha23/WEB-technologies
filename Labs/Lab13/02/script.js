...
function makeRequest(url){
    ...
    xhttp.onreadystatechange = function(){
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            ...
            arrSoil = [];
            if (items[0].selected) {
                for (let i = 0; i < textInf.getElementsByTagName("name").length; i++) {
                    arrComp.push(new Company(textInf.getElementsByTagName("name")[i].innerHTML, textInf.getElementsByTagName("acc")[i].innerHTML, textInf.getElementsByTagName("inc")[i].innerHTML))
                }
            } else {
                for (let i = 0; i < textInf.getElementsByTagName("item").length; i++) {
                    arrSoil.push(new SoilArea(textInf.getElementsByTagName("cip")[i].innerHTML, textInf.getElementsByTagName("area")[i].innerHTML, textInf.getElementsByTagName("price")[i].innerHTML, textInf.getElementsByTagName("soiltype")[i].innerHTML))
                }
            }
            out();
        }
