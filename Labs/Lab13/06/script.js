...
function makeRequest(url){
    ...
    xhttp.onreadystatechange = function(){
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            textInf = xhttp.responseText.split("\n").map(function(x){return x.split(";")});
            arrComp = [];
            arrSoil = [];
            if (items[0].selected) {
                for (let i = 0; i < textInf.length; i++) {
                    arrComp.push(new Company(textInf[i][0], textInf[i][1], textInf[i][2]))
                }
            } else {
                for (let i = 0; i < textInf.length; i++) {
                    arrSoil.push(new SoilArea(textInf[i][0], textInf[i][1], textInf[i][2], textInf[i][3]))
                }
            }
            out();
        }
