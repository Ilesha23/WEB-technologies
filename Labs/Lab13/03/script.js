        ...
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            ...
            if (items[0].selected) {
                for (let i = 0; i < textInf.getElementsByTagName("item").length; i++) {
                    arrComp.push(new Company(textInf.getElementsByTagName("item")[i].getAttribute("name"), textInf.getElementsByTagName("item")[i].getAttribute("acc"), textInf.getElementsByTagName("item")[i].getAttribute("inc")))
                }
            } else {
                for (let i = 0; i < textInf.getElementsByTagName("item").length; i++) {
                    arrSoil.push(new SoilArea(textInf.getElementsByTagName("item")[i].getAttribute("cip"), textInf.getElementsByTagName("item")[i].getAttribute("area"), textInf.getElementsByTagName("item")[i].getAttribute("price"), textInf.getElementsByTagName("item")[i].getAttribute("soiltype")))
                }
            }
            out();
        }
