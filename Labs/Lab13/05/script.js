...
function makeRequest(url){
    ...
    xhttp.onreadystatechange = function(){
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            textInf = JSON.parse(xhttp.responseText);
            arrComp = [];
            arrSoil = [];
            if (items[0].selected) {
                for (let i = 0; i < textInf.items.length; i++) {
                    arrComp.push(new Company(textInf.items[i].name, textInf.items[i].acc, textInf.items[i].inc))
                }
            } else {
                for (let i = 0; i < textInf.items.length; i++) {
                    arrSoil.push(new SoilArea(textInf.items[i].cip, textInf.items[i].area, textInf.items[i].price, textInf.items[i].soiltype))
                }
            }
            out();
        }
    ...
    if (items[0].selected) {
        xhttp.open("GET", "file1.json", true);
    } else {
        xhttp.open("GET", "file2.json", true);
    }
    xhttp.send(null);
}