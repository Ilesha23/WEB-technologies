...
//
$('#statis').click(function (e) { 
    e.preventDefault();
    //alert($('#table >tbody >tr :eq(1) >td').eq(1).html())
    //alert($('#table >tbody >tr >td').length)
    //alert(parseInt($('#table >tbody >tr >td').eq(1)))
    let a = 0, b = 0, c = 0, d = 0;
    if ($('select').val() == 1) {
        for(let i = 0; i < $('#table >tbody >tr >td').length; i+=3){
            a += parseFloat($('#table >tbody >tr >td').eq(i+1).html());
            c += parseFloat($('#table >tbody >tr >td').eq(i+2).html());
            d += parseFloat($('#table >tbody >tr >td').eq(i+1).html());
            b++;
        }
        $('#stat').html(`Inc avg: ${a/b}<br>Acc avg: ${c/b}<br>Inc sum: ${d}`);
    }
    else {
        for (let i = 0; i < $('#table >tbody >tr >td').length; i+=4) {
            a += parseFloat($('#table >tbody >tr >td').eq(i+1).html());
            c += parseFloat($('#table >tbody >tr >td').eq(i+2).html());
            d += parseFloat($('#table >tbody >tr >td').eq(i+1).html());
            b++;
        }
        $('#stat').html(`Area avg: ${a/b}<br>Price avg: ${c/b}<br>Area sum: ${d}`);
    }
});
