...
$('#sav').click(function (e) { 
    ...
    if ($('select').val() == 1) {
        let a;
        for(let i = 0; i < $('#table >tbody >tr').length*3; i++){
            if ($(`#table >tbody >tr :eq(${i})`).css('color') == 'rgb(255, 0, 0)') {
                a = i/3 - 1;
                break;
            }
        }
        arrComp.splice(a + 1, 1, new Company($('#f1 >input').eq(0).val(), $('#f1 >input').eq(1).val(), $('#f1 >input').eq(2).val()));
        showTable();
    }
});
